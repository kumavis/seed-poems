...
function generatePoem () {
  const poem = Array(3).fill().map(() => generateSentence()).join('\n')
  return poem
}

function generateSentence () {
  const subject = selectRandom(nouns)
  const adverb = selectRandom(adverbs)
  const verb = selectRandom(verbs)
  const object = selectRandom(nouns)
  
  const sentence = `The ${subject} ${adverb} ${verb}(s) the ${object}`
  return sentence
}
...
function loadAdverbs () {
  const rawAdverbs = fs.readFileSync('./adv_ed.txt', 'utf8')
  const adverbs = rawAdverbs.split('\n').slice(0, 2048)
  return adverbs
}
